      ...
      <div className="relative pt-12 pb-12 px-6 overflow-hidden">
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row gap-10 items-center md:items-start relative z-10">
          ...
          <div className="relative">
            <div className="w-32 h-32 rounded-[40px] bg-black/[0.03] dark:bg-white/5 p-1 border border-black/10 dark:border-white/10 shadow-2xl transition-all">
            ...
            <div className="absolute -bottom-2 -right-2 w-10 h-10 bg-amber-primary rounded-2xl flex items-center justify-center border-4 border-background shadow-xl">
          ...
          <div className="flex-1 text-center md:text-left space-y-4">
            <div>
              <div className="flex flex-wrap justify-center md:justify-start items-center gap-4 mb-2">
                <h1 className="text-4xl font-display font-bold">{profile.full_name || profile.username || 'Fotografer'}</h1>
              ...
              <div className="flex flex-wrap justify-center md:justify-start items-center gap-4 text-xs font-mono text-muted">
                <span className="flex items-center gap-1.5"><UserIcon className="w-3 h-3" /> @{profile.username}</span>
                <span className="flex items-center gap-1.5"><MapPin className="w-3 h-3" /> {profile.location || 'Basecamp belum diatur'}</span>
              </div>
            </div>

            <p className="max-w-lg text-sm text-muted leading-relaxed italic opacity-80">
              "{profile.bio || 'Mulai ceritamu di sini...'}"
            </p>

            <div className="flex flex-wrap justify-center md:justify-start gap-4">
              <div className="flex items-center gap-2 px-3 py-1.5 bg-black/[0.03] dark:bg-white/5 rounded-xl border border-black/10 dark:border-white/10">
          ...
          <div className="grid grid-cols-2 gap-4 w-full md:w-auto">
            <div className="p-4 panel text-center min-w-[100px]">
      ...
      <div className="max-w-5xl mx-auto px-6">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex gap-2">
            ...
            <button className="p-2 bg-black/[0.03] dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-lg text-muted"><ImageIcon className="w-4 h-4" /></button>
          </div>
          
          <button 
            onClick={() => setIsEditing(true)}
            className="px-6 py-2 bg-black/[0.03] dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl text-xs font-bold flex items-center gap-2 hover:border-amber-primary transition-all"
          >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {filteredPhotos.length > 0 ? (
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity p-4 flex flex-col justify-end">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[8px] font-medium text-white/60 uppercase tracking-tighter line-clamp-1">
            ...
            <div className="col-span-full py-32 text-center border-2 border-dashed border-black/10 dark:border-white/10 rounded-[40px]">
              <Camera className="w-12 h-12 mx-auto mb-4 opacity-10" />
              <p className="text-muted font-medium italic">"Mulai Hunting Pertamamu"</p>
              <Link href="/map" className="inline-block mt-6 px-8 py-3 bg-black/[0.03] dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl text-sm font-bold hover:border-amber-primary transition-all">
      ...
      <AnimatePresence>
        {isEditing && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            >
              <div className="p-6 border-b border-black/10 dark:border-white/10 flex items-center justify-between sticky top-0 bg-background z-10">
                ...
                <button 
                  onClick={() => setIsEditing(false)}
                  className="p-2 hover:bg-black/[0.03] dark:hover:bg-white/5 rounded-full transition-colors"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="p-8 overflow-y-auto flex-1 space-y-8">
                ...
                <div className="p-4 bg-black/[0.03] dark:bg-white/5 rounded-2xl flex gap-3 items-start">
              ...
              <div className="p-6 border-t border-black/10 dark:border-white/10 flex gap-3 sticky bottom-0 bg-background z-10">
                <button 
                  onClick={() => setIsEditing(false)}
                  className="flex-1 py-3 border border-black/10 dark:border-white/10 rounded-xl font-bold text-sm hover:bg-black/5 dark:hover:bg-white/5 transition-all"
                >
                  Batal
                </button>
                <button 
